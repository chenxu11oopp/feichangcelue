<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:73:"E:\www\feichangcelue/application/index\view\ucenter\mobile\real_name.html";i:1528449928;}*/ ?>
<!doctype html>
<html>
	<head>
		<meta charset="UTF-8">
	...
	<body class="realName_body">
		...
		<div class="mui-content">
		   <p class="ml-tip color_red">请准确填写身份信息，以便绑定银行卡和提款。</p>
			<form class="mui-input-group" action="<?php echo url('index/ucenter/doReanNameAuth'); ?>" method="post">
			    <div class="mui-input-row">
			        <label>真实姓名：</label>
			        <input id="realName" type="text" name="realName" class="mui-input-clear" placeholder="请输入您的真实姓名">
			    </div>
			    <div class="mui-input-row">
			        <label>身份证号：</label>
			        <input id="idCard" type="text" name="IDNumber" class="mui-input-clear" placeholder="请输入您的身份证号">
			    </div>
				<button type="submit" id="realName_btn" class="ml_btn mui-btn mui-btn-block">确  认</button>
			</form>
		...
		<script type="text/javascript">
	        ...
	        $("#realName_btn").on('tap',function () {
	            ...
	            /*$.post("<?php echo url('index/ucenter/doReanNameAuth'); ?>", {realName:$('#realName').val(), IDNumber:$('#idCard').val()}, function(data){
	                        $("#realName").val('');
	                        $("#idCard").val('');
	                        console.log(data);
	                        if(data.code != '0'){
